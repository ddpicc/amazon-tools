...
export default async function ReviewsToolPage({ searchParams }: { searchParams: { projectId?: string } }) {
  ...
  return <div className="mx-auto max-w-3xl"><p className="font-label text-xs uppercase tracking-[0.2em] text-[var(--md-primary)]">Review insights</p><h1 className="mt-3 font-headline text-3xl font-bold">评论洞察</h1><p className="mt-2 font-label text-sm text-[var(--md-on-surface-variant)]">通过 Reveyes 获取本次 Amazon 公开评论，按你的筛选条件展示全部结果并生成 AI 总结；不会创建监控订阅。</p><ReviewInsightsForm project={project ? { id: project.id, marketplace: project.marketplace, asins: project.trackedAsins.map((item) => item.asin) } : undefined} /></div>;
}
