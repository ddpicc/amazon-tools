  ...
  return (
    <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-zinc-50">API 用量</h2>
          <p className="mt-1 text-sm text-zinc-400">按项目记录 Sorftime request 消耗，后续可用于套餐和成本控制。</p>
        </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        {usage.byApi.map((item) => (
          <div key={item.apiName} className="rounded-xl border border-zinc-800 bg-zinc-950/70 p-4">
            <p className="font-medium text-zinc-100">{item.apiName}</p>
          ...
          <tbody className="divide-y divide-zinc-800">
            ...
            {usage.recentLogs.map((log) => (
              <tr key={log.id} className="text-zinc-200 hover:bg-zinc-950/40">
                <td className="px-4 py-3">{log.apiName}</td>
